...
ROUND_WEIGHTS = {"Love": 1 / 3, "Career": 1 / 3, "Chaos": 1 / 3}
ASPECTS = {
    "conjunction": (0, 14),
    "sextile": (60, 9),
    "square": (90, -12),
...
def _round(
    name: str, p1: dict[str, dict[str, object]], p2: dict[str, dict[str, object]]
) -> ScoredRound:
    hits: list[AspectHit] = []
        ...
        if result:
            ...
            hits.append(
                AspectHit(
                    ...
                    aspect_name,
                    orb,
                    value * pair_weight * (0.65 + 0.35 * closeness),
                )
            )
    ...
    if name == "Chaos":
        ...
        if (
            sun1
            and sun2
            and ELEMENTS.get(str(sun1.get("sign"))) != ELEMENTS.get(str(sun2.get("sign")))
        ):
            compatibility = max(0, compatibility - 7)

    # The edge uses only chart facts: the relevant planets' absolute speeds.
    speed1 = sum(
        abs(float(p1[planet].get("speed", 0))) for planet, _ in PAIR_WEIGHTS[name] if planet in p1
    )
    speed2 = sum(
        abs(float(p2[planet].get("speed", 0))) for _, planet in PAIR_WEIGHTS[name] if planet in p2
    )
    ...
    labels = tuple(
        hit.label
        for hit in sorted(hits, key=lambda item: abs(item.weighted_value), reverse=True)[:3]
    )
    return ScoredRound(name, p1_score, p2_score, compatibility, labels)


def score_battle(
    p1_chart: dict[str, object], p2_chart: dict[str, object]
) -> tuple[list[ScoredRound], int, str]:
