...
def test_chart_preview_returns_ephemeral_known_person_fighter() -> None:
    response = TestClient(app).post("/chart-preview", json={
        "name": "Partner", "dob": "1994-08-21", "tob": None, "tobUnknown": True,
        "place": "Pune, India", "lat": 18.5204, "lon": 73.8567,
        "tz": "Asia/Kolkata", "tone": "straight", "lang": "en", "source": "web",
    })
    assert response.status_code == 200
    payload = response.json()
    assert payload["name"] == "Partner"
    assert payload["chartMode"] == "solar"
    assert set(payload["stats"]) == {"Love", "Career", "Luck", "Fire", "Chaos"}
